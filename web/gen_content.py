...
SYS = ("You write concise, genuinely useful, non-spammy copy for a dental "
       "directory website that helps patients find dentists. Plain, trustworthy "
       "tone. No emojis, no hype, no made-up "
...
def _groq(prompt: str) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json",
                 ...
                 "User-Agent": "dentists-directory/1.0"},
    )
...
def prompt_category(name, n):
    return (f"Dental specialty: '{name}'. "
            "Return JSON {\"intro\": <120-180 word paragraph explaining what this "
            "type of dentist does, what treatments they provide, when to see one, "
            "and what to look for when choosing one>, "
            "\"faq\": [3 items of {\"q\":..., \"a\":<40-70 words>}]}.")


def prompt_city(city, state, n, cats):
    return (f"City: {city}, {state}. Dental specialties available: "
            f"{', '.join(cats)}. Return JSON {{\"intro\": <120-180 word paragraph "
            "about choosing a dentist in this specific city — general guidance on "
            "finding a good local dentist, what to check in reviews, and booking "
            "an appointment>, \"faq\": [3 items of {\"q\":..., \"a\":<40-70 words>}]}.")
